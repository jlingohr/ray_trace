...
impl RGBSpectrum {
    ...
    pub fn from_xyz(xyz: [Float; 3]) -> RGBSpectrum {
        let c = xyz_to_rgb(&xyz);
        RGBSpectrum { c }
    }

    // Creates RGB spectrum from an abritrarily sampled SPD by converting the spectrum to
    // XYZ and then to RGB and then evaluates the piecewise linear sampled spectrum at
    // 1-nm steps
    pub fn from_sampled(lambda: &[Float], v: &[Float], n: usize) -> RGBSpectrum {
        let mut xyz = [0.0; 3];
    ...
    pub fn to_rgb(&self) -> [Float; 3] {
        let rgb = [self.c[0], self.c[1], self.c[2]];
        rgb
    }
    ...
    pub fn y(&self) -> Float {
        self.c[0] * 0.212671 + self.c[1] * 0.715160 + self.c[2] * 0.072169
    }
